---
loaded: active-fetch   # consulted when d-land actually fires. Key → dialectic/loaded-status-frontmatter.md.
---

# Landing-protocol — the `d-land` checklist

> Single-home for `d-land` (`DYAD.md` §Channel discipline points here, does not restate this).
> **Status: our own candidate, un-settled, dialectic-tier — not yet a Playbook** (Ontology: a
> Ledger of proofs precedes a Playbook; this hasn't lived long enough to have one).

## Provenance — read the caveat before trusting this as doctrine

Adapted from `dyad-bond` (`GLOSSARY.md` §Dyad-UI cluster + `dialectic/relationship-craft.md`
§The landing-discipline) — **referenced, not copied**, per the existing Covalent-gate precedent
for this class of cross-dyad borrowing (dyad-bond/steward/healer — their route).

**This is not a settled artifact at the source either.** Bond's own copy lives in `dialectic/`
(their live/under-falsification tier — explicitly distinct from `kb/`, their settled-and-safe-
to-cite tier; no landing-discipline entry exists in bond's `kb/`). Bond's own GLOSSARY.md entry
is tagged `⚠️ CANDIDATE / not yet wired`, and their checklist's own falsification-status note
says it **failed its own first audit**: checked against the two PRs actually cut that session,
only items 4 and 5 (below) held reliably both times; item 6 never held either time. Bond's
verdict on their own copy: *"NOT kb-eligible until it survives at least one clean, fully-
verified run."* We are adopting this as **our own open experiment** to rub against our own
landings — not as proven doctrine borrowed from a proven source.

## The checklist

1. **Verify green *this run*** — `read:` before asserting ready, not from a stale pass.
2. **Scope to the whole session-arc** — `git log origin/main..HEAD`, not just the triggering
   commit. One PR per arc.
3. **Open mechanically once signalled** — no second CTA once the Operator's `d-land` has fired.
4. **Write a real Summary + Test-plan body**, not a bare PR.
5. **Never merge, auto-merge, or request reviewers** — stop at "exists and is described." This
   is Gatekeeping-is-a-gate-act + `no-self-ratify`, already ours, named at the PR boundary
   specifically. Merge is a *separate*, explicit Operator gate act, never inferred from the
   land-token alone.
6. **Surface it back explicitly** — in chat *and* a durable record (rack/Ledger), never left
   implicit in chat alone.
7. **Flag a restart-implication** if the Anchor itself (`DYAD.md`) was touched.
8. **Re-check for a PR template each session**, not once.

## Falsification status (ours, to be updated as we run it)

Un-audited here as of adoption (cycle-43). Re-audit after the next few landings: does the full
checklist hold, or does an item fail once the obvious ones are being watched for — the same
question bond's own audit asked of their copy. Not eligible to promote out of `dialectic/` until
it survives at least one clean, fully-verified run here, independently of whether bond's ever does.

## Lived note

PR #19 (this repo, cycle-43) was opened **and** merged in one pass, before this protocol existed
— that merge stands as pre-adoption. `d-land` from here forward stops at open.
